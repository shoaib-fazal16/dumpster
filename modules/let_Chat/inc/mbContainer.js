...
(function ($) {
    jQuery.fn.buildContainers = function (options) {
        return this.each(function () {
            ...
            if ($.metadata) {
                ...
                if (container.metadata().content) container.attr("content", container.metadata().content);
                if (container.metadata().aspectRatio) container.attr("aspectRatio", container.metadata().aspectRatio);
                if (container.metadata().grid) container.attr("grid", container.metadata().grid);
                if (container.metadata().gridx) container.attr("gridx", container.metadata().gridx);
                if (container.metadata().gridy) container.attr("gridy", container.metadata().gridy);
                if (container.metadata().handles) container.attr("handles", container.metadata().handles);
            ...
            if (container.attr("content")) container.mb_changeContainerContent(container.attr("content"));
    ...
    jQuery.fn.containerResize = function () {
        ...
        $(this).resizable({
            ...
            stop: function () {
                ...
                $(this).find(".c:first , .mbcontainercontent:first").css({
                    height: elHeight
                });
        ...
        if (buttons != "") {
            ...
            for (var i in btn) {
                ...
                if (btn[i] == "m") {
                    ...
                    $(this).find(".collapsedContainer:first").bind("click", function () {
                        container.containerCollapse(opt)
                    })
                    // letrium v
                    switch(expand_collapse_chat){
                    	case 'Double click':
                    		$(this).find(".n:first").bind("dblclick", function () {
				                container.containerCollapse(opt)
				            })
                    		break;
                    		
                    	case 'One click':
                    		$(this).find(".n:first").bind("click", function () {
                    			if (move_trigger){
                    				container.containerCollapse(opt)
                    			}
				            })
            ...
            if (fadeOnClose) $(this).find(".buttonBar:first img").css({
                opacity: .5,
                cursor: "pointer",
                "mozUserSelect": "none",
                ...
                if (fadeOnClose) $(this).fadeTo(200, .5)
            });
    ...
    jQuery.fn.containerCollapse = function (opt) {
        this.each(function () {
            ...
            if ($(this).attr("collapsed") == "false") {
                ...
                if (opt.minimizeEffect == "fade") container.find(".o:first").fadeOut(opt.effectDuration, function () {});
                else {
                    container.find(".icon:first").hide();
                    container.find(".o:first").slideUp(opt.effectDuration, function () {});
                    container.animate({
                        height: container.find(".n:first").outerHeight() + container.find(".s:first").outerHeight()
                ...
                if (opt.minimizeEffect == "fade") container.find(".o:first").fadeIn(opt.effectDuration, function () {});
                else {
    ...
    jQuery.fn.containerIconize = function (opt) {
        ...
        return this.each(function () {
            ...
            this.dockIcon = $("<img src='" + opt.elementsPath + "icons/" + (container.attr("icon") ? container.attr("icon") : "restore.png") + "' class='restoreContainer' width='46'/>").appendTo(dockPlace).css("cursor", "pointer").hide().attr("contTitle", container.find(".n:first").html()).bind("click", function () {
                container.attr("iconized", "false");
                if (container.is(".draggable")) container.css({
                    top: $(this).offset().top,
    ...
    jQuery.fn.mb_resizeTo = function (h, w) {
        ...
        $(this).animate({
            ...
            $(this).find(".c:first , .mbcontainercontent:first").animate({
                height: elHeight
            })
        })
    };
